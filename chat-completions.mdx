...
### Full request body example (with field annotations)
...
| --- | --- | --- | --- |
| `model` | string (enum) | Yes | Model identifier to use. Current options: `surf-ask` / `surf-research` / `surf-1.5`. |
| `messages` | array\<object\> | Yes | List of chat messages. Recommended: include at least one `role=user` message; optionally include one `role=system` message for system instructions. |
| `stream` | boolean | No | Enable streaming output. When `true`, returns SSE (`text/event-stream`); when `false` (default), returns JSON. |
| `reasoning_effort` | string (enum) | No | Reasoning strength: `low` / `medium` / `high`. Higher values typically improve reasoning quality, but may be slower / consume more tokens. |
| `ability` | array\<string\> (enum) | No | Surf extension: hints/constraints for which capability domains are available for this request. Options: `search` / `evm_onchain` / `solana_onchain` / `market_analysis` / `calculate`. |
| `citation` | array\<string\> (enum) | No | Surf extension: citation formats to include in the output. Options: `source` / `chart`. |
